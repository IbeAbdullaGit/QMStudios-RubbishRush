...
bool Application::LoadScene(const std::string& path) {
	if (std::filesystem::exists(path)) {
		...
		Gameplay::Scene::Sptr _currentScene = Gameplay::Scene::Load(path);
...
void Application::LoadScene(const Gameplay::Scene::Sptr& _currentScene) {
...
void Application::_Run()
{
	...
	// Done loading, app is _currentScene running!
	_isRunning = true;
	...
	//fetch resources
	GameObject::Sptr trashyM = _currentScene->FindObjectByName("Trashy");
	GameObject::Sptr binM = _currentScene->FindObjectByName("Bin");
	GameObject::Sptr RectangleE = _currentScene->FindObjectByName("Rec");
	GameObject::Sptr TrashyE = _currentScene->FindObjectByName("TrashyE");
	...
	//setup trash
	MeshResource::Sptr trashMesh = _currentScene->FindObjectByName("Trash1")->Get<RenderComponent>()->GetMeshResource();

	// Create our material
	Material::Sptr trashMaterial = _currentScene->FindObjectByName("Trash1")->Get<RenderComponent>()->GetMaterial();

	//UI
	GameObject::Sptr startMenu = _currentScene->FindObjectByName("Start");
	GameObject::Sptr pauseMenu = _currentScene->FindObjectByName("Pause");
	GameObject::Sptr failMenu = _currentScene->FindObjectByName("Fail");
	GameObject::Sptr winMenu = _currentScene->FindObjectByName("Win");
	GameObject::Sptr UIText = _currentScene->FindObjectByName("Time Text");
	GameObject::Sptr trashRemainder = _currentScene->FindObjectByName("Trash Remaining");
	GameObject::Sptr objective = _currentScene->FindObjectByName("Objective UI Canvas");
	GameObject::Sptr returnUI = _currentScene->FindObjectByName("Return Feedback");
	GameObject::Sptr submitUI = _currentScene->FindObjectByName("Submit Feedback");
	...
	while (_isRunning) {
		// Handle _currentScene switching
		...
		if (_currentScene != nullptr) {
			...
			else if (timeleveltDone && _currentScene->IsPlaying && start)
			{
				if (lose)
				{
					...
					if (glfwGetKey(GetWindow(), GLFW_KEY_SPACE)) //return to game
					{
						...
						GameObject::Sptr trash1 = _currentScene->FindObjectByName("Trash1");
						GameObject::Sptr trash2 = _currentScene->FindObjectByName("Trash2");
						GameObject::Sptr trash3 = _currentScene->FindObjectByName("Trash3");
						GameObject::Sptr trash4 = _currentScene->FindObjectByName("Trash4");
						if (trash1 != nullptr)
						{
						...
						//cup collection
						{
							GameObject::Sptr trashM = _currentScene->CreateGameObject("Trash1");
							{
							...
							GameObject::Sptr trash2 = _currentScene->CreateGameObject("Trash2");
							{
							...
							GameObject::Sptr trash3 = _currentScene->CreateGameObject("Trash3");
							{
							...
							GameObject::Sptr trash4 = _currentScene->CreateGameObject("Trash4");
							{
			...
			Camera::Sptr camera = _currentScene->MainCamera;
...
void Application::_HandleSceneChange() {
	// If we currently have a current _currentScene, let the layers know it's being unloaded
	if (_currentScene != nullptr) {
	...
	// Let the layers know that we've loaded in a new _currentScene
	for (const auto& layer : _layers) {
	...
	// Wake up all game objects in the _currentScene
	_currentScene->Awake();
